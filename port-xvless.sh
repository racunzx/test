...
echo -e "     [2]  Change Port XRAY Vless None TLS $none"
echo -e "     [x]  Exit"
echo -e "======================================"
echo -e ""
...
if [[ -z $cek ]]; then
sed -i "s/$tls/$tls1/g" /usr/local/etc/xray/vless.json
...
;;
2)
read -p "New Port XRAY Vless None TLS: " none1
if [ -z $none1 ]; then
echo "Please Input Port"
exit 0
fi
...
if [[ -z $cek ]]; then
sed -i "s/$none/$none1/g" /usr/local/etc/xray/vnone.json
